...
public class Projectile : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        else if (collision.TryGetComponent<BreakablePart>(out var attackable))
        {
            attackable.TakePartDamage(transform.position);
        }

        if ((!_payload.isPiercing &&
            _prefabToReturn != null &&
            IsInLayerMask(collision.gameObject.layer, _hitReturnLayers) &&
            (didHitEnemy || collision.TryGetComponent<BreakablePart>(out _)))
