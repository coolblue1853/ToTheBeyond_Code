...
public sealed class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private SkillSO dashBuffSkill;

    // 던전 내 화폐
    public PlayerCoinHandler coinHandler;
    ...
    private void Awake()
    {
        ...
        coinHandler = GetComponent<PlayerCoinHandler>();
    ...
    private void Update()
    {
        ...
        {
            if ((SettingsManager.Instance != null && SettingsManager.Instance.IsSettingsOpen) || PermanentUpgradeManager.Instance.IsUpgradeUIOpen)
                return;
        ...
        {
            PermanentUpgradeManager.Instance.AddCurrency(500);
        }
    ...
    private void OnDash(InputValue value)
    {
        ...
        if (value.isPressed && _dash.TryDash(movement.FacingDirection))
        {
            _weaponHandler.equippedWeapon?.OnDash();

            if (dashBuffSkill != null )
            {
                StatusEffectManager.Instance?.ApplyEffectFromSkill(dashBuffSkill, runtimeStat.DashCooldown);
            }
        }
    }
    ...
    public void ResetPlayerState()
    {
        ...
        gravityCtrl.Restore();
        PermanentUpgradeManager.Instance.ApplyUpgradeToPlayer();
        isControllable = true;
    }
