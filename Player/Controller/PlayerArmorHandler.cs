...
public class PlayerArmorHandler : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        playerController._playerHealth = GetComponent<PlayerHealth>();
    ...
    public void CheckArmorElemet(ArmorElement newElement)
    {
        ...
        if (elementController.armorDictionary.ContainsKey(newElement._type))
        {
            ...
            if (tempSprite != null)
                ...
                newElement.spriteRenderer.sprite = _basicSprite;
    ...
    public void SwapArmor(WorldArmor worldArmor) // 아머 교체
    {
        ...
        {
            equippedArmor.Unequip(); // 패시브 제거 → PlayerHealth가 Δ만큼 현재 체력 감소 처리
        }
        ...
        equippedArmor.Equip(playerController.runtimeStat); // 패시브 적용 → PlayerHealth가 Δ만큼 현재 체력 증가 처리
